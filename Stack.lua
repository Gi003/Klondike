...
function Stack:drag_move(active_card, x, y)
    ...
    for i, card in ipairs(self.card_stack) do
        ...
        if (enable_move == true) then 
            card:drag_move(x,y + (self.data.y_interval * count))
            count = count + 1
        end
    end
end
...
function Stack:update_position()
    if self.data.display == "spread" then 
        for i=1, self:num_cards() do 
            self.card_stack[i]:move(self.data.x, self.data.y + (self.data.y_interval* (i-1)))
        end
        self.cap:move(self.data.x, self.data.y + (self.data.y_interval* (self:num_cards())))
    else 
        for i=1, self:num_cards() do 
